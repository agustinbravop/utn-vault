Las _ideas de negocio_ surgen de la innovación y se transforman en _oportunidades_ de negocio cuando satisfacen una necesidad del cliente o nos diferencian de los demás competidores.

## Innovación

> La **innovación** es el análisis sistemático de los cambios para transformarlos en oportunidades de negocio. - Drucker

La innovación es un proceso iterativo para entregar un nuevo mercado y/o nuevo servicio y/o avance tecnológico. Un invento se vuelve _innovador_ cuando el mercado lo valida y compra.

Tipos de innovación:

1. Producto.
2. Proceso.
3. Organizativa.
4. Marketing.

Grados de innovación:

- **Incremental** (mejorar algo que ya existe).
- **Radical** (lanzar algo nuevo, distinto).
- **Disruptiva**.

**La innovación genera valor**. El objetivo es aumentar la productividad y la competitividad de la empresa. Es importante la innovación abierta en todos los roles de la organización, no solo en el departamento de investigación y desarrollo (R&D).

Fuentes de innovación:

- Cambios en mercados e industrias.
- Necesidad de proceso.
- Incongruencias.
- Acontecimientos inesperados.
- Cambios demográficos.
- Cambios en la percepción.

Habilidades de un innovador:

- Reconocer patrones.
- Preguntar.
- Observar.
- Experimentar.
- Crear redes.
- Paciencia.
- Tomar riesgos.
- Confianza.

El proceso de innovación es 1% inspiración y 99% transpiración.

## Ideas de Negocio

Una idea de negocio es una descripción corta y específica de lo que será un negocio. Hay que encontrar un enfoque diferente o un concepto que todavía no haya sido explotado en el mercado. Tenemos que diferenciarnos de los demás.

Para que la idea sea también una oportunidad, debe:

1. Satisfacer una necesidad.
2. Diferenciarse de los demás.
3. Crear valor para el cliente.

Técnicas para generar ideas:

- **Racional**: anotar necesidades insatisfechas y analizar soluciones.
- **Brainstorming**: reunirse en grupo, definir un objetivo, y establecer tiempo límite.
- **Asociación forzosa**: pensar en un producto ya existente, escribir palabras al azar, y asociar las palabras al producto.

Para elegir una idea de negocio, hay que evaluarlas a todas según ciertos criterios. Elegimos la mejor y planificamos su implementación.
